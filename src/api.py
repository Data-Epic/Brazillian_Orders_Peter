...
def api_load_sellers_data() -> jsonify:
    """
    This API endpoint loads the Sellers data into the duckdb database

    Args:
        None

    Returns:
        JSON: A JSON response containing the status of the request
        ...
        if file_upload["status"] == "success":
            ...
            try:
                ...
                if loaded_data["status"] == "success":
                    ...
                    # if the csv file to be loaded do not have specific columns, return error
                    sellers_df_columns = [
                        "seller_id",
                    ...
                    if not all(col in list_dicts_df[0] for col in sellers_df_columns):
                        return jsonify(
                            {
                                "error": "Invalid file, Kindly provide the Sellers csv file"
                            }
                        ), 400

                with get_db() as db:
                    try:
                        existing_sellers, existing_seller_ids = query_existing_data(
                            Sellers, list_dicts_df, db
                        )
                        new_sellers = []
                        ...
                        sellers_list = []
                        sellers_list = jsonify_loaded_data(
                            new_sellers, sellers_list, Sellers
                        )
                ...
                logger.error(
                    f"Error loading and transforming data to a list of dataframes: {str(e)}"
                )
...
def api_get_sellers() -> jsonify:
    """
    This API endpoint retrieves all the Sellers data from the duckdb database
    ...
    with get_db() as db:
        try:
            ...
            return jsonify(
                {"status": "error", "error": f"retrieving sellers data: {str(e)}"}
            ), 500


@app.route("/api/load_customers_data", methods=["POST"])
def api_load_customers_data() -> jsonify:
    """
    This API endpoint loads the Customers data into the duckdb database
            ...
            try:
                ...
                if loaded_data["status"] == "success":
                    ...
                    if not all(col in list_dicts_df[0] for col in customers_df_columns):
                        return jsonify(
                            {
                                "error": "Invalid file, Kindly provide the Customers csv file"
                            }
                        ), 400

                with get_db() as db:
                    try:
                        existing_customers, existing_customer_ids = query_existing_data(
                            Customers, list_dicts_df, db
                        )
                        ...
                        for record in list_dicts_df:
                            if record["id"] not in existing_customer_ids:
                                customer = Customers(
                                    ...
                                    customer_zip_code_prefix=record[
                                        "customer_zip_code_prefix"
                                    ],
                        ...
                        customers_list = []
                        customers_list = jsonify_loaded_data(
                            new_customers, customers_list, Customers
                        )
                ...
                logger.error(
                    f"Error loading the dataset, kindly provide the valid customers dataset: {str(e)}"
                )
                return jsonify({"status": "error", "error": str(e)}), 400
...
def api_get_customers() -> jsonify:
    """
    This API endpoint retrieves all the Customers data from the duckdb database
    ...
    with get_db() as db:
        try:
            ...
            return jsonify(
                {"status": "error", "error": f" retrieving Customers data: {str(e)}"}
            ), 500


@app.route("/api/load_orders_data", methods=["POST"])
def api_load_orders_data() -> jsonify:
    """
    This API endpoint loads the Orders data into the duckdb database
    ...
    try:
        ...
        if file_upload["status"] == "success":
            ...
            try:
                ...
                with get_db() as db:
                    try:
                        existing_orders, existing_order_ids = query_existing_data(
                            Orders, list_dicts_df, db
                        )
                        ...
                        for record in list_dicts_df:
                            if record["id"] not in existing_order_ids:
                                order = Orders(
                                    ...
                                    order_purchase_timestamp=record[
                                        "order_purchase_timestamp"
                                    ],
                                    order_approved_at=record["order_approved_at"],
                                    order_delivered_carrier_date=record[
                                        "order_delivered_carrier_date"
                                    ],
                                    order_delivered_customer_date=record[
                                        "order_delivered_customer_date"
                                    ],
                                    order_estimated_delivery_date=record[
                                        "order_estimated_delivery_date"
                                    ],
                                )
                        ...
                        return jsonify(
                            {
                                "status": "error",
                                "error": f"Database operation failed details {str(db_error)}",
                            }
                        ), 500

            except Exception as e:
                logger.error(
                    f"Error loading the dataset, kindly provide the valid orders dataset: {str(e)}"
                )
                return jsonify({"status": "error", "error": str(e)}), 400
...
def api_get_orders() -> jsonify:
    """
    This API endpoint retrieves all the Orders data from the duckdb database
    ...
    with get_db() as db:
        try:
            ...
            return jsonify(
                {"status": "error", "error": f"error retrieving Orders data: {str(e)}"}
            ), 500


@app.route("/api/load_order_items_data", methods=["POST"])
def api_load_order_items_data() -> jsonify:
    """
    This API endpoint loads the Order Items data into the duckdb database
        ...
        if file_upload["status"] == "success":
            ...
            try:
                ...
                if loaded["status"] == "success":
                    ...
                    if not all(
                        col in list_dicts_df[0] for col in order_items_df_columns
                    ):
                        return jsonify(
                            {
                                "error": "Invalid file, Kindly provide the Order Items csv file"
                            }
                        ), 400

                with get_db() as db:
                    try:
                        existing_order_items, existing_order_ids = query_existing_data(
                            Order_Items, list_dicts_df, db
                        )
                        new_order_items = []
                        for record in list_dicts_df:
                            if record["id"] not in existing_order_ids:
                        ...
                        order_items_list = []
                        order_items_list = jsonify_loaded_data(
                            new_order_items, order_items_list, Order_Items
                        )
                ...
                logger.error(
                    f"Error loading the dataset, kindly provide the valid order items dataset: {str(e)}"
                )
...
def api_get_order_items() -> jsonify:
    """
    This API endpoint retrieves all the Order Items data from the duckdb database
    ...
    with get_db() as db:
        try:
            ...
            return jsonify(
                {
                    "status": "error",
                    "error": f"error retrieving Order Items data: {str(e)}",
                }
            ), 500


@app.route("/api/load_order_payments_data", methods=["POST"])
def api_load_order_payments_data() -> jsonify:
    """
    This API endpoint loads the Order Payments data into the duckdb database
        ...
        if file_upload["status"] == "success":
            ...
            try:
                ...
                if loaded_data["status"] == "success":
                    ...
                    if not all(
                        col in list_dicts_df[0] for col in order_payments_df_columns
                    ):
                        return jsonify(
                            {
                                "error": "Invalid file, Kindly provide the Order Payments csv file"
                            }
                        ), 400

                with get_db() as db:
                    try:
                        existing_order_payments, existing_order_ids = (
                            query_existing_data(Order_Payments, list_dicts_df, db)
                        )
                        new_order_payments = []
                        for record in list_dicts_df:
                            if record["id"] not in existing_order_ids:
                        ...
                        order_payments_list = []
                        order_payments_list = jsonify_loaded_data(
                            new_order_payments, order_payments_list, Order_Payments
                        )
                        ...
                        return jsonify(
                            {
                                ...
                                "existing_order_payments_count": len(
                                    existing_order_payments
                                ),
                            }
                        ), 200
                ...
                logger.error(
                    f"Error loading the dataset, kindly provide the valid order payments dataset: {str(e)}"
                )
                return jsonify({"status": "error", "error": str(e)}), 400
...
def api_get_order_payments() -> jsonify:
    """
    This API endpoint retrieves all the Order Payments data from the duckdb database
    ...
    with get_db() as db:
        try:
            ...
            return jsonify(
                {
                    "status": "error",
                    "error": f"error retrieving Order Payments data {str(e)}",
                }
            ), 500


@app.route("/api/load_products_data", methods=["POST"])
def api_load_products_data() -> jsonify:
    """
    This API endpoint loads the Products data into the duckdb database
            ...
            try:
                ...
                if loaded_data["status"] == "success":
                    ...
                    if not all(col in list_dicts_df[0] for col in products_df_columns):
                        return jsonify(
                            {
                                "error": "Invalid file, Kindly provide the Products csv file"
                            }
                        ), 400

                with get_db() as db:
                    try:
                        existing_products, existing_product_ids = query_existing_data(
                            Products, list_dicts_df, db
                        )
                        ...
                        for record in list_dicts_df:
                            if record["id"] not in existing_product_ids:
                                product = Products(
                                    ...
                                    product_description_lenght=record[
                                        "product_description_lenght"
                                    ],
                        ...
                        products_list = []
                        products_list = jsonify_loaded_data(
                            new_products, products_list, Products
                        )
                ...
                logger.error(
                    f"Error loading the dataset, kindly provide the valid products dataset: {str(e)}"
                )
...
def api_get_products() -> jsonify:
    """
    This API endpoint retrieves all the Products data from the duckdb database
    ...
    with get_db() as db:
        try:
            ...
            return jsonify(
                {
                    "status": "error",
                    "error": f"error retrieving Products data: {str(e)}",
                }
            ), 500


@app.route("/api/load_products_category", methods=["POST"])
def api_load_products_category() -> jsonify:
    """
    This API endpoint loads the Products Category data into the duckdb database
        ...
        if file_upload["status"] == "success":
            ...
            try:
                ...
                if loaded_data["status"] == "success":
                    ...
                    product_category_df_columns = [
                        "product_category_name",
                        "product_category_name_english",
                    ]
                    if not all(
                        col in list_dicts_df[0] for col in product_category_df_columns
                    ):
                        return jsonify(
                            {
                                "error": "Invalid file, Kindly provide the Product Category csv file"
                            }
                        ), 400

                with get_db() as db:
                    try:
                        existing_product_categories, existing_product_category_ids = (
                            query_existing_data(Product_Category, list_dicts_df, db)
                        )
                        ...
                        for record in list_dicts_df:
                            if record["id"] not in existing_product_category_ids:
                                product_category = Product_Category(
                                    ...
                                    product_category_name_english=record[
                                        "product_category_name_english"
                                    ],
                                )
                        ...
                        product_category_list = jsonify_loaded_data(
                            new_product_categories,
                            product_category_list,
                            Product_Category,
                        )
                        ...
                        return jsonify(
                            {
                                ...
                                "new_product_categories_count": len(
                                    new_product_categories
                                ),
                                "existing_product_categories_count": len(
                ...
                logger.error(
                    f"Error loading the dataset, kindly provide the valid product category dataset: {str(e)}"
                )
...
def api_get_products_category() -> jsonify:
    """
    This API endpoint retrieves all the Products Category data from the duckdb database
    ...
    with get_db() as db:
        try:
            product_categories, product_categories_list = query_table_data(
                Product_Category, db
            )
            ...
            logger.error(
                f"An error occurred retrieving all product categories: {str(e)}"
            )
            return jsonify(
                {
                    "status": "error",
                    "error": f"error retrieving Product Categories data: {str(e)}",
                }
            ), 500


@app.route("/api/process_fact_table", methods=["POST"])
def api_process_fact_table() -> jsonify:
    """
    This API endpoint processes the fact table by joining all the dimension tables on common keys
    and inserting the data into the fact table. It loads the fact table data into the duckdb database
    ...
    try:
        with get_db() as db:
            try:
                list_of_tables = process_dim_tables(
                    db,
                    Sellers,
                    Customers,
                    Orders,
                        ...
                        return jsonify(
                            {
                                ...
                                "message": "transformed_data didnt yield a valid polars dataframe",
                            }
                        ), 400

                fact_table = process_fact_table(list_of_tables)
                fact_table = fact_table.filter(not fact_table.is_duplicated())

                if not isinstance(fact_table, pl.DataFrame):
                    return jsonify(
                        {
                            "status": "error",
                            "message": "fact_table didnt yield a valid polars dataframe",
                        }
                    ), 400

                list_dicts_df = df_to_list_of_dicts(fact_table)
                existing_fact_table, existing_fact_table_ids = query_existing_data(
                    FactTable, list_dicts_df, db
                )
                ...
                fact_table_list = []
                fact_table_list = jsonify_loaded_data(
                    new_fact_table, fact_table_list, FactTable
                )
...
def api_get_fact_table() -> jsonify:
    """
    This API endpoint retrieves all the Fact Table data from the duckdb database
...
def api_load_top_sellers() -> jsonify:
    """
    This API endpoint loads the Top Sellers data into the duckdb database
    ...
    try:
        with get_db() as db:
            try:
                ...
                if not fact_table:
                    return jsonify(
                        {"error": "Fact table to be analyzed not found in the database"}
                    ), 404
                ...
                existing_top_sellers, existing_top_sellers_ids = query_existing_data(
                    Top_Sellers, list_dicts_df, db
                )
                ...
                top_sellers_list = []
                top_sellers_list = jsonify_loaded_data(
                    new_top_sellers, top_sellers_list, Top_Sellers
                )
...
def api_get_top_sellers() -> jsonify:
    """
    This API endpoint retrieves all the Top Sellers data from the duckdb database
...
def api_load_top_selling_product_category() -> jsonify:
    """
    This API endpoint loads the Top Selling Product Category data into the duckdb database
    ...
    try:
        with get_db() as db:
            try:
                ...
                fact_table = process_dim_table_df(fact_table)
                top_selling_product_category_df = get_top_selling_product_category(
                    fact_table
                )
                ...
                (
                    existing_top_selling_product_category,
                    existing_top_selling_product_category_ids,
                ) = query_existing_data(Top_Selling_Product_Category, list_dicts_df, db)
                ...
                for record in list_dicts_df:
                    if record["id"] not in existing_top_selling_product_category_ids:
                        ...
                        new_top_selling_product_category.append(
                            top_selling_product_category
                        )
                ...
                top_selling_product_category_list = jsonify_loaded_data(
                    new_top_selling_product_category,
                    top_selling_product_category_list,
                    Top_Selling_Product_Category,
                )
                return jsonify(
                    {
                        ...
                        "new_top_selling_product_category_count": len(
                            new_top_selling_product_category
                        ),
                        "existing_top_selling_product_category_count": len(
                            existing_top_selling_product_category
                        ),
                    }
                ), 200
...
def api_get_top_selling_product_category() -> jsonify:
    """
    This API endpoint retrieves all the Top Selling Product Category data from the duckdb database
    ...
    with get_db() as db:
        try:
            top_selling_product_category, top_selling_product_category_list = (
                query_table_data(Top_Selling_Product_Category, db)
            )
            ...
            logger.error(
                f"An error occurred retrieving all top selling product category: {str(e)}"
            )
            return jsonify(
                {"error retrieving Top Selling Product Category data": str(e)}
            ), 500


@app.route("/api/load_orders_status_analysis", methods=["POST"])
def api_load_orders_status_analysis() -> jsonify:
    """
    This API endpoint loads the Orders Status Analysis data into the duckdb database
    ...
    try:
        with get_db() as db:
            try:
                ...
                existing_orders_status_analysis, existing_orders_status_analysis_ids = (
                    query_existing_data(Order_Status_Count, list_dicts_df, db)
                )
                ...
                orders_status_analysis_list = jsonify_loaded_data(
                    new_orders_status_analysis,
                    orders_status_analysis_list,
                    Order_Status_Count,
                )
                ...
                return jsonify(
                    {
                        ...
                        "new_orders_status_analysis_count": len(
                            new_orders_status_analysis
                        ),
                        "existing_orders_status_analysis_count": len(
                            existing_orders_status_analysis
                        ),
                    }
                ), 200
...
def api_get_orders_status_analysis() -> jsonify:
    """
    This API endpoint retrieves all the Orders Status Analysis data from the duckdb database
    ...
    with get_db() as db:
        try:
            orders_status_analysis, orders_status_analysis_list = query_table_data(
                Order_Status_Count, db
            )
            ...
            logger.error(
                f"An error occurred retrieving all orders status analysis: {str(e)}"
            )
            return jsonify(
                {"error retrieving Orders Status Analysis data": str(e)}
            ), 500


@app.route("/api/load_average_delivery_duration", methods=["POST"])
def api_load_average_delivery_duration() -> jsonify:
    """
    This API endpoint loads the Average Delivery Duration data into the duckdb database
    ...
    try:
        with get_db() as db:
            try:
                ...
                (
                    existing_orders_delivery_analysis,
                    existing_orders_delivery_analysis_ids,
                ) = query_existing_data(
                    Top_Average_Delivery_Duration, list_dicts_df, db
                )
                # Insert only new orders_delivery records
                ...
                for record in list_dicts_df:
                    if record["id"] not in existing_orders_delivery_analysis_ids:
                        orders_delivery_analysis = Top_Average_Delivery_Duration(
                            ...
                            average_delivery_duration_days=record[
                ...
                orders_delivery_analysis_list = jsonify_loaded_data(
                    new_orders_delivery_analysis,
                    orders_delivery_analysis_list,
                    Top_Average_Delivery_Duration,
                )
                ...
                return jsonify(
                    {
                        ...
                        "new_orders_delivery_analysis_count": len(
                            new_orders_delivery_analysis
                        ),
                        "existing_orders_delivery_analysis_count": len(
                            existing_orders_delivery_analysis
                        ),
                    }
                ), 200
...
def api_get_average_delivery_duration() -> jsonify:
    """
    This API endpoint retrieves all the Average Delivery Duration data from the duckdb database
    ...
    with get_db() as db:
        try:
            orders_delivery_analysis, orders_delivery_analysis_list = query_table_data(
                Top_Average_Delivery_Duration, db
            )
            ...
            logger.error(
                f"An error occurred retrieving all orders delivery analysis: {str(e)}"
            )
            return jsonify(
                {"error retrieving Orders Delivery Analysis data": str(e)}
            ), 500


@app.route("/api/analyze_loyal_customers", methods=["POST"])
def api_load_loyal_customers() -> jsonify:
    """
    This API endpoint loads the Loyal Customers data into the duckdb database
    ...
    try:
        with get_db() as db:
            try:
                ...
                existing_loyal_customers, existing_loyal_customers_ids = (
                    query_existing_data(Loyal_Customers, list_dicts_df, db)
                )
                ...
                loyal_customers_list = jsonify_loaded_data(
                    new_loyal_customers, loyal_customers_list, Loyal_Customers
                )
...
def api_get_loyal_customers() -> jsonify:
    """
    This API endpoint retrieves all the Loyal Customers data from the duckdb database
    ...
    with get_db() as db:
        try:
            loyal_customers, loyal_customers_list = query_table_data(
                Loyal_Customers, db
            )
